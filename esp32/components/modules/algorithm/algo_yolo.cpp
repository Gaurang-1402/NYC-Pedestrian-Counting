    ...
    while (true)
    {
        ...
        if (gEvent)
        {
            ...
            if (xQueueReceive(xQueueFrameI, &frame, portMAX_DELAY))
            {
                ...
                uint32_t something = output->dims->data[0];
                uint32_t records = output->dims->data[1]; // i think this is the anount fo poepel found that was wrong it shows up as 2268
                uint32_t num_class = output->dims->data[2] - OBJECT_T_INDEX; // this showedd as 1
                int16_t num_element = num_class + OBJECT_T_INDEX; //this may be the unique ids it wasnt i dont think it showed as 6
                ...
                matrixA.setZero();
                std::cout << "i compiled unbelievabel" << matrixA;
